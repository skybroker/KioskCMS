...
If Action="Save" Then
	...
	For i = LBound(AryID) To UBound(AryID)
		...
		Sql="Select NavOrder From JobClass Where NavParent="&NavParent&" Order By NavOrder Desc"
		...
		'获取导航条深度值
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select NavLevel From JobClass Where ID="&NavParent&""
		Rs.Open Sql,Conn,1,1
		If Not (Rs.Eof Or Rs.Bof) Then
		...
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select * From JobClass Where ID="&AryID(i)&""
		...
		End If
	Next
	Response.Write("<script>alert('\u7c7b\u522b\u79fb\u52a8\u64cd\u4f5c\u6210\u529f\u3002');window.location.href='Job_Class.asp?ID="&NavParent&"';</script>")
	Response.End()
End If
%>
...
For i = LBound(AryID) To UBound(AryID)
	If IsNumeric(AryID(i))=True Then
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select NavTitle From JobClass Where ID="&AryID(i)&""
		Rs.Open Sql,Conn,1,1
...
<td width="75%"><input type="submit" value="保 存" class="Button"> <input type="button" value="返 回" class="Button" onClick="window.location.href='Job_Class.asp'"></td>
</tr>
</table>
</form>
</td>
