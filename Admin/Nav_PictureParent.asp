...
If Action="Save" Then
	...
	For i = LBound(AryID) To UBound(AryID)
		...
		Sql="Select NavOrder From SitePicture Where NavParent="&NavParent&" Order By NavOrder Desc"
		...
		'获取导航条深度值
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select NavLevel From SitePicture Where ID="&NavParent&""
		Rs.Open Sql,Conn,1,1
		If Not (Rs.Eof Or Rs.Bof) Then
		...
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select * From SitePicture Where ID="&AryID(i)&""
		...
		End If
	Next
	Response.Write("<script>alert('\u56fe\u7247\u5185\u5bb9\u6240\u5c5e\u7236\u7c7b\u522b\u79fb\u52a8\u64cd\u4f5c\u6210\u529f\u3002');window.location.href='Nav_Picture.asp?ID="&NavParent&"';</script>")
	Response.End()
End If
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Nav_Picture.asp">导航条内容维护</a> >> 更改导航条所属父类别</td>
...
For i = LBound(AryID) To UBound(AryID)
	If IsNumeric(AryID(i))=True Then
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select NavTitle From SitePicture Where ID="&AryID(i)&""
		Rs.Open Sql,Conn,1,1
...
<%=GetSelect("SitePicture",0)%>
</select></td>
</tr>
<tr>
<td class="Right" width="25%" align="right">&nbsp;</td>
<td width="75%"><input type="submit" value="保 存" class="Button"> <input type="button" value="返 回" class="Button" onClick="window.location.href='Nav_Picture.asp'"></td>
</tr>
</table>
</form>
</td>
