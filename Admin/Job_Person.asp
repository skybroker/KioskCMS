...
<%
Call ISPopedom(UserName,"Job_Person")
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Job_Person.asp">人才列表</a></td>
...
<th width="12%" class="Right">应聘岗位</th>
<th width="12%" class="Right">姓　　名</th>
<th width="12%" class="Right">联系方式</th>
<th width="20%" class="Right">应聘时间</th>
<th width="12%" class="Right">状　　态</th>
<th width="12%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From JobPersonInfo order by NavOrder asc"
...
if rs("cZt")="0" then
	  stra="<font color='FF0000'>末读</font>"
  else
	  stra="已读"
  end if
%>
...
<td class="Right"><a href="Job_Person_Show.asp?ID=<%=Rs("ID")%>"><%= rs("gangwei") %></a></td>
<td class="Right"><%= rs("xinmin") %>&nbsp;</td>
<td class="Right" align="center"><%= rs("dianhua") %></td>
<td class="Right" align="center"><%= FormatDateTime(rs("tTime"),1)%></td>
<td class="Right" align="center"><%= stra %></td>
<td><a href="Job_Person_Show.asp?ID=<%=Rs("ID")%>">查看</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u4fe1\u606f\u5206\u7c7b\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
