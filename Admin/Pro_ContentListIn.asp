...
Case "InIndex"
	...
	Response.End()
Case "UnTure"
	Conn.Execute("Update ShopInfo Set ShopVisit=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "UnFalse"
	Conn.Execute("Update ShopInfo Set ShopVisit=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "IsLock"
	...
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('产品下架操作成功，确定后返回列表页面。');window.location.href='?Page="&Page&"&ClassID="&ClassID&"&ShowType="&ShowType&"';</script>")
	Response.End()
Case "Delete"
	Page=ReplaceBadChar(Trim(Request("Page")))
	AryID = Split(ID,",")
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:60%">当前位置：<a href="Pro_ContentListIn.asp">上架产品维护</a>
<%
If ShopName<>"" Then
...
End If
%> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="Pro_ContentListIn.asp">页面刷新</a></td>
...
<td width="75%" valign="top">以下为所有上架产品（PRODUCTS）的信息列表，同时您可以对发布的产品进行下架到现货供应（STORAGE）里。<br>
...
Sql="Select * From ShopInfo Where ShopLock=0 "&StrSql&" And ClassID In ("&ClassID&GetAllChild("ShopClass",ClassID)&") Order By ShopOrder Desc"
...
<th width="24%" class="Right">产品名称<a href="#" onClick="top.CreateNewTab('Pro_ContentAdd.asp?ClassID=<%=ClassID%>','Pro_ContentAdd','添加商品')">[+]</a></th>
<th width="25%" class="Right">产品类别</th>
<th width="12%" class="Right">产品排序</th>
<th width="14%" class="Right">最新产品<span style="color:#f00">(仅会员可见)</span></th>
...
<td class="Right"><a href="Pro_ContentEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ClassID=<%=ClassID%>&FileName=Pro_ContentListIn&ShowType=<%=ShowType%>&ShopName=<%=Server.URLEncode(ShopName)%>"><%=Rs("ShopName")%></a></td>
...
If Rs("ShopVisit")="0" Then
Response.Write("<a href=""?Action=UnTure&ID="&Rs("ID")&""">否</a>")
Else
Response.Write("<a href=""?Action=UnFalse&ID="&Rs("ID")&""" style=""color:red"">是</a>")
End If
%></td>
<td class="Right">
<%
...
%>
<tr>
<th colspan="2" style="font-weight:normal">操作：<a href="javascript:IsLock();" style="font-weight:normal">下架到现货供应</a>&nbsp;┊&nbsp;<a href="javascript:Delete();" style="font-weight:normal">删除</a>&nbsp;┊&nbsp;<a href="javascript:ChangeParent();" style="font-weight:normal">转移</a>&nbsp;┊&nbsp;<a href="Pro_ContentListInFind.asp?ClassID=<%=ClassID%>&ShowType=<%=ShowType%>" style="font-weight:normal">查找</a></th>
