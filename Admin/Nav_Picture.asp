...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From SitePicture Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update SitePicture Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update SitePicture Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From SitePicture Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update SitePicture Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update SitePicture Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From SitePicture Where ID In ("&AryID(i)&GetAllChild("SiteExplain",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('\u5220\u9664\u64cd\u4f5c\u6210\u529f\uff0c\u786e\u5b9a\u540e\u8fd4\u56de\u56fe\u7247\u5185\u5bb9\u5217\u8868\u9875\u9762\u3002');window.location.href='?Page="&Page&"&ID="&ParentID&"';</script>")
	Response.End()
End Select
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Nav_Picture.asp">友情链接维护</a><%=GetNavPath("SitePicture",ID)%></td>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="Nav_PictureAdd.asp?ID=<%=ID%>">添加友情链接</a>&nbsp;</td>
</tr>
<tr>
...
<td width="90%" valign="top">以下为系统所有的图片内容的信息列表。<br>
...
<th width="21%" class="Right">友情链接名称<a href="Nav_PictureAdd.asp?ID=<%=ID%>">[+]</a></th>
<th width="40%" class="Right">友情链接地址</th>
<th width="12%" class="Right">友情链接排序</th>
<th width="12%" class="Right">友情链接状态</th>
<th width="10%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From SitePicture Where NavParent="&ID&" Order By NavOrder Asc"
...
<td class="Right"><a href="Nav_PictureEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>"><%=Rs("NavTitle")%></a></td>
<td class="Right">http://<%=Rs("NavRemark")%>&nbsp;</td>
...
End If
%></td>
<td><a href="Nav_PictureEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u56fe\u7247\u5185\u5bb9\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
...
function ChangeParent()
{
    ...
    if (confirm('\u786e\u5b9a\u8981\u66f4\u6539\u9009\u4e2d\u7684\u56fe\u7247\u5185\u5bb9\u7684\u6240\u5c5e\u7236\u7c7b\u522b\u5417\uff1f')) {
        window.location.href = 'Nav_PictureParent.asp?ID=' + l;
    }
}
function Delete() {
    var l = GetAllChecked();
