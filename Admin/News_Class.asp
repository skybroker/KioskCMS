...
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From NewsClass Where ID In ("&AryID(i)&GetAllChild("ShopClass",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="News_ClassAdd.asp?ID=<%=ID%>">添加信息分类</a>&nbsp;</td>
</tr>
<tr>
...
<th width="25%" class="Right">类别名称<a href="News_ClassAdd.asp?ID=<%=ID%>">[+]</a></th>
<th width="40%" class="Right">类别说明</th>
<th width="10%" class="Right">类别排序</th>
<th width="10%" class="Right">类别状态</th>
<th width="10%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select ID,NavTitle,NavRemark,NavOrder,NavParent,NavLock From NewsClass Where NavParent="&ID&" Order By NavOrder Asc"
...
<td class="Right"><a href="News_ClassEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>"><%=Rs("NavTitle")%></a></td>
