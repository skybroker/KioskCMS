...
If Action="Save" Then
	...
	For i = LBound(AryID) To UBound(AryID)
		...
		Sql="Select NavOrder From SiteNav Where NavParent="&NavParent&" Order By NavOrder Desc"
		...
		'获取导航条深度值
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select NavLevel From SiteNav Where ID="&NavParent&""
		Rs.Open Sql,Conn,1,1
		If Not (Rs.Eof Or Rs.Bof) Then
		...
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select * From SiteNav Where ID="&AryID(i)&""
		...
		End If
	Next
	Response.Write("<script>alert('\u5bfc\u822a\u6761\u6240\u5c5e\u7236\u7c7b\u522b\u79fb\u52a8\u64cd\u4f5c\u6210\u529f\u3002');window.location.href='Nav_Content.asp?ID="&NavParent&"';</script>")
	Response.End()
End If
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Nav_Content.asp">导航条内容维护</a> >> 更改导航条所属父类别</td>
...
For i = LBound(AryID) To UBound(AryID)
	If IsNumeric(AryID(i))=True Then
		Set Rs=Server.CreateObject("Adodb.RecordSet")
		Sql="Select NavTitle From SiteNav Where ID="&AryID(i)&""
		Rs.Open Sql,Conn,1,1
