...
function SetIframe()
{
    try{
        ...
        //$(loadingIframeId).style.height = $(loadingIframeId).document.body.scrollHeight + "px";
        //$(loadingIframeId).style.width = $(loadingIframeId).document.body.scrollWidth + "px";
    ...
    if(!ChkObjectIsExists(id))
    {
        ...
        CreateTabTitle(id,title);      
         ...
         if(id != "Default"){
            ...
            if(preTab == "FileManager" || preTab == "DataBaseAdmin" || id == "EnterpriseCard"){
...
function SetOtherTabpageStyle(id,flag)
{
    ...
    for(var i=0;i<iframeList.length;i++)
    {
        ...
        else
        {
			...
			//Edit By Apollo 2009-8-14
            /*if(id == "FileManager" || id == "DataBaseAdmin" || id == "EnterpriseCard"){
                ...
                iframeList[i].style.height = "100%";
			}*/
			iframeList[i].style.width = "100%";
			iframeList[i].style.height = "480px";
        }
    }    
    ...
    try{
        ...
        try{
            if(loadingIframeId != "Default" && loadingIframeId != "LmInfo"){
                var list = $(loadingIframeId).contentWindow.document.getElementsByTagName("TABLE");
	...
	//Edit By Apollo 2009-08-14
    /*if(loadingIframeId == "FileManager" || loadingIframeId == "DataBaseAdmin" || loadingIframeId == "EnterpriseCard"){
        $(loadingIframeId).style.width = $("div_Main_Content").offsetWidth - 10 + "px";
    }*/
	$(loadingIframeId).style.width = $("div_Main_Content").offsetWidth - 10 + "px";
    
    //设置打开页面的页面文本框聚焦    
    if(loadingIframeId != "DomainBindings"&&
        loadingIframeId != "Permissions"&&
        loadingIframeId != "Filters")
    {
        try{
            var list = $(loadingIframeId).contentWindow.document.getElementsByTagName("INPUT");
            for(var i=0;i<list.length;i++){
                if(list[i].type == "text"){
                    list[i].focus();
                    break;
...
function CreateTabTitle(id,title)
{
    ...
    try
    {
        if(!ChkObjectIsExists("cp_tab_Title_Div_" + id))
        {
            ...
            divItem.innerHTML = "<a href='#' onclick='SetTabTitleStyle(\"" + id + "\")'>" 
                              + title 
                              + "</a>"
...
function SetTabTitleStyle(id)
{
    ...
    for(var i=0;i<divList.length;i++)
    {
        if(divList[i].className == "curr")
        {
            ...
            var close = "<a onmouseover=\"ChangeCloseBtn('"+obj.id+"',1)\" onmouseout=\"ChangeCloseBtn('"+obj.id+"',2)\" href=\"javascript:DeleteTabTitle('" + obj.id.replace("cp_tab_Title_Div_","") + "')\"><img id=\""+obj.id+"_icon_nav_close\" src=\"images/icon-nav-close.gif\" border=\"0\" /></a>&nbsp;&nbsp;";
            var line = "<img src=\"images/nav-curr-line.gif\" align=\"absmiddle\" /><div></div>";
            
            if(obj.id == "cp_tab_Title_Div_Default")
            {
                var temp1 = list.split('>');
                obj.innerHTML = "<img src=\"images/icon-index.gif\" /><a href='#' onclick='SetOtherTabpageStyle(\"" + obj.id.replace("cp_tab_Title_Div_","") + "\")'>"
                                + temp1[1] 
                                + "</a>&nbsp;&nbsp;&nbsp;&nbsp;"
                                + line;
            }
            else
            {
                ...
                obj.innerHTML = "<a href='#' onclick='SetOtherTabpageStyle(\"" + obj.id.replace("cp_tab_Title_Div_","") + "\")'>"
                              + temp2[0] 
                              + "</a>"
                              + close 
                              + line;
    ...
    {
        text = "<img src=\"images/icon-index.gif\" /> "
             + text;
    }
    else
    {
        text += " <a onmouseover=\"ChangeCloseBtn('"+selectTab.id+"',1)\" onmouseout=\"ChangeCloseBtn('"+selectTab.id+"',2)\" href=\"javascript:DeleteTabTitle('" + id + "')\"><img id=\""+selectTab.id+"_icon_nav_close\" src=\"images/icon-nav-close.gif\" border=\"0\" /></a>";
    }
...
function CompletePlanBar()
{
 ...
 function ShowFunction(){
    if(loadingIframeId == "Default"){
        $("iframeFunctionVote").src = "FunctionVote.aspx?init=true&t1=&t2=&fun="+loadingIframeId;
    }
 }
 ...
 */
 function ResizeTab(){
    ...
    for(var i=0;i<list.length;i++){
        if(list[i].length>0){
            $(list[i]).style.display = "none";
            if(currentPos==0 && list[i]=="cp_tab_Title_Div_" + loadingIframeId){
                currentPos = i;
            }
        }
    }
    ...
    //divOperateMark
    var divOperateMark = $(loadingIframeId).contentWindow.document.getElementById("divOperateMark");
